...
class GridWorldEnv(gym.Env):
    def __init__(self):
        ...
        self.num_obstacles = 10  # Number of obstacles to place
    ...
    def reset(self):
        ...
        while len(self.obstacles) < self.num_obstacles:
            ...
            if rand_pos not in self.obstacles and rand_pos != self.runner_pos and rand_pos != self.tagger_pos:
    ...
    def _generate_obstacles(self):
        self.obstacles = []
        while len(self.obstacles) < self.num_obstacles:
            obstacle_pos = (random.randint(0, self.grid_size - 1), random.randint(0, self.grid_size - 1))
            # Ensure the obstacle is not where the runner or tagger starts
            if obstacle_pos not in self.obstacles and obstacle_pos != tuple(self.runner_pos) and obstacle_pos != tuple(self.tagger_pos):
    ...
    def step(self, action):
        ...
        new_runner_pos = [min(max(self.runner_pos[0] + move[0], 0), self.grid_size - 1), min(max(self.runner_pos[1] + move[1], 0), self.grid_size - 1)]

        if new_runner_pos not in self.obstacles:
            self.runner_pos = new_runner_pos

        # Tagger
        tagger_move = self._get_tagger_move()
        new_tagger_pos = [min(max(self.tagger_pos[0] + tagger_move[0], 0), self.grid_size - 1), min(max(self.tagger_pos[1] + tagger_move[1], 0), self.grid_size - 1)]

        if new_tagger_pos not in self.obstacles:
            self.tagger_pos = new_tagger_pos
    ...
    def render(self):
        ...
        if self.termin_var:
            ...
            for obstacle in self.obstacles:
                plt.scatter(obstacle[1], obstacle[0], color='gray', s=100, label='Obstacle')
