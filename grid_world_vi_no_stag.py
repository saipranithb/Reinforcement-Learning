...
class GridWorldEnv(gym.Env):
    ...
    def step(self, action):
        ...
        state = (self.runner_pos[0], self.runner_pos[1], self.tagger_pos[0], self.tagger_pos[1])
        ...
        new_runner_pos[0] = min(max(self.runner_pos[0] + move[0], 0), self.grid_size - 1)
        new_runner_pos[1] = min(max(self.runner_pos[1] + move[1], 0), self.grid_size - 1)

        # Tagger
        tagger_move = self._get_tagger_move()
        new_tagger_pos[0] = min(max(self.tagger_pos[0] + tagger_move[0], 0), self.grid_size - 1)
        new_tagger_pos[1] = min(max(self.tagger_pos[1] + tagger_move[1], 0), self.grid_size - 1)

        terminated = self.runner_pos == self.tagger_pos
        reward = -100 if terminated else -1
        
        if new_runner_pos in self.history:
            reward -= 30  # Apply penalty for toggling/oscillating


        self.history.append(self.runner_pos[:])
